...
class ChatMemories(Base):
    ...
    id :Mapped[int] = mapped_column(primary_key=True)
    chatId:Mapped[str] = mapped_column(String ,nullable=False )  #email+contact uuid_type
    ...
    sender_text: Mapped[str] = mapped_column(Text)    # what the actual user (Dev) typed
    contact_text: Mapped[str] = mapped_column(Text)   # what the person on the other end typed
    embedding : Mapped[list[float]] = mapped_column(Vector(768) , nullable=False)  #Adjust this NUMBER LATER⚠️⚠️⚠️⚠️⚠️    




async def UserWtsAcc_Write(user_id_:int ,  
                           WtAcc_:str,email_:str,   
...
async def ChatMemo_Write(chatId:str ,   
    ...
    async with Async_session() as Session :
        try :
            Users_chat_memo = ChatMemories(chatId = chatId, 
...
async def Read_UserWtsAcc(email:str):
    try:
        async with Async_session() as Session:
            ...
            data = Fetch_data.scalar_one_or_none()
            if  data:
                return data
            return data
    except:
